...
CONDITION_DOCS: list[dict] = [
    ...
    dict(check='conformance-structure', sig='not populated with numbered clauses', applies='both', condition='Each conformance profile scope yields at least one extracted clause identifier', pulls="numbered sub-headings and paragraph-leading 'Clause N'/bracketed target-id labels within the Conformance section span, scoped per named profile", compares_to="handbook-WPQualityRequirements.txt: 'a set of numbered conformance clauses to which any implementation must adhere'; BLOCKER at cs/os, WARN at wd/csd (fail-closed either way, never a silent pass on an empty section)"),
