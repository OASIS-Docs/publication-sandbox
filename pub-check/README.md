...
`docs.oasis-open.org`. Run it yourself before you submit. Anything it
flags, you fix while the document is still in your hands; a package that
passes will not be sent back by TC Administration for corrections, because
...
- **No configuration.** Every expectation is derived from the package
  ...
  publish path.
- **98 individual checks across 35 check classes**, and the set grows.
...
- [../examples/eox-core-v1.0-csd01/](../examples/eox-core-v1.0-csd01/):
  the real Validation Report from a real publication.
...
## Provenance: over a year of OASIS publications

The acceptance criteria are not invented; they are distilled. They combine
more than a year of real OASIS publication work: work products published,
promoted, and audited across CSAF, KMIP, PKCS#11, OpenEoX, NIEM, Akoma
Ntoso and LegalDocML, DMLex, UBL, Electronic Court Filing, STIX, OSLC,
Virtio, DPS, ACAL, and OpenDocument, authored in Markdown, Word, ODT,
DocBook/XML, and LaTeX. Every check in the gate traces to a real correction
round somewhere in that body of work:

- an editor placeholder that shipped and stayed live for years
- a mixed-case citation that 404ed on the case-sensitive host
- a PDF rendered from an older draft than the HTML beside it
- front matter citing files that never shipped
...
- a 13-package regression corpus of real submissions, in their original
  received form, from more than one authoring track
- one known-bad release candidate in that corpus whose 13 blockers the gate
  reproduces exactly as the manual intake review found them
- a 12-month retrospective over the year's intake, as received, which
  surfaced live defects on docs.oasis-open.org that had survived every
  human pass; those became checks too

The set grows. Every future correction round that surfaces a new failure
mode becomes a new acceptance criterion, and the catalog and advertised
counts regenerate from the code (`--list-checks` asserts them), so the
criteria in force are always exactly the ones the tool runs.
...
Every OASIS publication now ships two manifest artifacts, generated by the
...
HTML and PDF, with the authoritative source alongside, at the canonical
URLs. The input formats are many and various; the output contract is fixed.
oasis-pub-check validates that bar. The full output suite (HTML checks: title,
...
for them is roadmap, calibrated the same way the first tracks were --
