...
A note on renderers: wkhtmltopdf is what this sandbox's workflows run, but
...
directory so every shipped file carries the publication date. That is
release management, not a transform.

Every defect class these transforms guard against (the lint series D1-D7 and the post-render assertions A1/A2) is also enforceable in a TC's own build before submission, via [pub-check](pub-check/): `python3 pub-check/pub_check.py <stage-dir>`.
